...
1. **★预签名 PoC(录屏方案命门,最先做)★**:写一小段脚本用注入的 `sub-congrove-oss` key 对 `s3api.ruciah.com`
   ...
   **结论:预签名直传方案成立,P2 按 §7.4b 实施,回退通道(流式代理)保留但预计用不上。★★
   ★★ 2026-08-03 复盘(两个坑,PoC 当初都漏了)★★
   ① **ACAO 逗号串**:桶 CORS 一条 rule 放两个 origin 时,Garage 把它们 join(", ") 塞进
   Access-Control-Allow-Origin——curl 不做 CORS 判定所以「非空」断言全绿,**浏览器字面比较后必判失败**
   (装没装 CA 都挂,真凶)。平台已按 origin 拆条并改进开桶模板(AI_Talks 0117/0118),
   PoC 的 2b 断言已收紧为「必须等于请求 Origin」,复验八项全绿。
   ② **公网入口层**:对 s3api 只做 TCP 透传(内网证书漏给公网用户)+ 对大请求体/长请求掐断(502)。
   归属网络侧且**用户已冻结不动**(0115/0118)——即「纯公网无 CA 设备」是既定现实,不是待修项。
   → 故 P2 的形态定为**三级降级**:预签直传(装 CA 的内网/WG 设备,最快)→ 同源分片代理
   (8MiB/片 + 每片 3 次重试,穿得过任意代理)→ 整文件 POST(仅小文件)。前端开局探测选路。★★
