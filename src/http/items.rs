...
//! 于是(版本恢复、原样重存、不同人各传一份同样的文件)天然去重。
//!
//! ⚠★这段头注 2026-08-15 才订正,它此前的后半句是本仓库最贵的一句注释★:
//!   原文写的是「key 带 item_id,**跨 item 不共享对象**,所以删 item 时只需对本 item 的
//!   key 做引用计数,★不会误删别人的★」。那是 2026-08-05 内容寻址改造**之前**的事实,
//!   改造之后 key 就是 `blobs/<sha>`,**同一个对象被任意多个 item 共享**。
//!   而这句话留在这儿,读的人(包括我)据此推出「这里删对象是安全的」——
//!   于是同一个洞前后出现三次(v0.4.38 projects::remove、v0.4.40 items::upload 收尾、
//!   media::complete 撞配额回滚),每一次都是把**别人的**那份 blob 打空:行还在、点开是空的。
//!   ★注释里「所以…是安全的」这种断言,过期时不会有任何东西提醒你,只会有人照着它写代码。★
//!
//! 现在的规矩:★删**内容寻址的 blob**(`blobs/<sha>`)只能走 `delete_unreferenced`★
//! (它按 items ∪ item_versions 全库数引用)。
//! ⚠ 例外只有**临时对象**:流式上传的 `tmp_key`、verify_sha 归位后的旧 key ——
//!   它们只有自己一个引用者,裸删是对的。判据是「这个 key 会不会被别人也指着」,
//!   不是「我这一处看起来安不安全」——★后一种判断法已经错过三次★。
//!
...
pub async fn owner_quota_used(pool: &sqlx::PgPool, owner: &str) -> AppResult<(i64, i64)> {
    ...
    .bind(owner)
    // ★这个兜底值现在超管可以在后台改★(2026-08-16):必须走 effective_default_quota,
    //   直接读常量的话「改了不生效」而且不报错。
