...
pub const APIS: &[Api] = &[
    ...
    //   ⚠ 它不是无害的重复:★一份读起来像权威、实际已经过期的文档★,
    //     照着它写调用的人会拿到一个对不上的响应形状。
    //   发现的过程也值得记:它在生产构建里**完全看不见** ——
    //   React 的「两个子元素用了同一个 key」警告只在 dev 模式发,
    ...
    api!("GET", "/api/projects/{id}/items", "内容", "≥viewer", "内容树(扁平表,前端按 parent_id 组树)", "",
         res: Vec<crate::http::items::ItemRow>),
    ...
    api!("GET", "/api/admin/users", "超管", "超管",
         "全部用户,含**生效配额**与 `quota_is_default`(true = 没单独设过,跟着全站默认走)", "",
         res: Vec<crate::http::admin::UserRow>),
    api!("PUT", "/api/admin/users/{username}/super", "超管", "超管", "设/撤超管位", "is_super",
         req: crate::http::admin::SuperIn, res: crate::http::admin::OkOut),
    ...
    api!("GET", "/api/admin/settings", "超管", "超管",
         "三项治理配置的**生效值 + 它从哪来**(`source`: db/env/default)。★source 不是调试信息★:「有人设成了 10」和「没人设过恰好默认 10」在改 env / 升级时表现完全不同",
         "",
         res: crate::http::admin::SettingsOut),
    api!("GET", "/api/admin/settings/default-quota/impact", "超管", "超管",
         "★改全站默认配额之前先算影响面★:跟随默认的人数 + 改完会立刻超额的人。没有它,「一键调小全站配额」就是个无法预估后果的按钮",
         "bytes",
         res: crate::http::admin::ImpactOut),
    api!("PUT", "/api/admin/settings/{key}", "超管", "超管",
         "改一项治理配置。key ★白名单★三个:project_creators / default_quota_bytes / default_remind_minutes —— 不设白名单的话将来任何新 key 都会自动变成可被外部写入",
         "value",
         req: crate::http::admin::SettingIn, res: crate::http::admin::SettingPutOut),
    api!("GET", "/api/admin/llm/models", "超管", "超管", "列网关当前**常驻**的模型 + 现在选的是哪个。★按需模型不在列表里但能调★,所以界面允许手输", "",
         res: crate::http::admin::ModelsOut),
