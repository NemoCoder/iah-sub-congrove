...
//!
//! ⚠ 本模块从 `http/meetings.rs` 抽出(2026-08-07):转移主持人也要发信之后,
//! 通知就不再是会议独有的了。把它留在 meetings 里会让 projects.rs 去调另一个 handler 模块的私有函数 ——
...
pub async fn notify_meeting(state: &AppState, mid: i64, targets: &[String], title: &str, body: &str) {
...
/// 项目相关的站内信(转移主持人等)。与会议版的区别只在 `ref` 前缀与直达链接的形状,
