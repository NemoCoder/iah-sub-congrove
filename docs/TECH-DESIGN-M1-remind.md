...
| 10 | 单场 NULL、个人默认也 NULL | ★按兜底 15 分钟发★（见下） |
| 11 | pod 停 2 小时后重启，期间有该发的 | ★不补发★（§3.2） |

⚠★用例 10 曾经与 §7② 自相矛盾★（2026-08-12 订正）：这张表是开放问题**还没关**的时候写的，
当时的假设是「都没设 = 不发」；而 §7② 在 2026-08-11 拍板了「兜底 15 分钟」，表却没跟着改。
两者只有一个能成立 —— ★以 §7② 为准★，因为「功能上线等于没上线」正是它要解决的问题：
没有偏好行的人本来一条提醒都收不到。
**这类矛盾要当成缺陷看**：如果照着这张表写测试，测试会绿，而绿的是**错的行为**；
门禁越机械，喂给它的判据写错的代价越大。
