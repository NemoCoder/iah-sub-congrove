...
# ⚠★这里必须带 E2E key★(2026-08-23 修):dev 通道的网关对**整个域名**做 SSO 门禁,
#   `/healthz` 也一样被 302 到 Keycloak —— 没有任何白名单例外。
#   ★我加这个端点时在代码注释里写「免鉴权是有意的」,那个前提从一开始就是错的★:
#   免的是 congrove 自己那层鉴权,网关那层免不了。
#   结果是这道闸连着几天报「问不到后端版本 —— 不能当成通过」,
#   而真相是**我没带凭证去问**。★一个自己造出来的假红,和假绿一样浪费人★
#   ——它让我以为部署卡住了,实际一直在跟进。
#   (量前端 bundle 那两行本来就带着 key,只有我新加的这行漏了。)
back=$(curl -sS --max-time 20 --cacert "$CA" -H "X-IAH-E2E-Key: $KEY" "$B/version" \
       | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
if [ -z "$back" ]; then
  # 拿不到 = 量不到,不是通过。老版本后端没有 /version 这个端点,升上去一次之后就有了。
  echo "期望 $want   前端 [$got]   后端 [取不到]"
  echo "★问不到后端版本(GET /version)—— 不能当成通过★"
  echo "  ① 线上还是 v0.7.13 及更早 → 它还没有这个端点,先把改动部署上去;"
  echo "  ② 没配 E2E key(~/.config/iah/congrove-e2e-key)→ dev 网关会把请求 302 到 Keycloak。"
  exit 2
fi
