...
scan() {
  ...
  # ④ ★中文残留：改名只换了 ASCII 的 meeting，没换用户看得见的「会」★（2026-08-12）
  #
  # 上面三条一条都没响，而线上文案里躺着「不再旁听这场会？」「这场会已取消」
  # 「一场会可关联多个项目」——★因为这道闸从头到尾只认 ASCII★。
  # 发现它们靠的是把界面文字从 DOM 里全量导出来自己读，不是靠 grep。
  #
  # ⚠★两条收窄，都是初版实跑时被假红逼出来的（写下来免得下一版又放宽）★：
  #
  #  (a) **只扫 `web/src/`**。初版扫全树，于是 `tests/api_cases.rs` 的用例描述
  #      （"他在公开项目 P 有一场会"）、`e2e/specs/*.ts` 的测试标题全被算成「用户可见文案」
  #      —— 十几条假红。★测试用例的名字不是产品文案★，它爱怎么写怎么写。
  #      代价：后端返给前端的报错文案（toast）扫不到。认了：那类是异常路径的短句，
  #      而这道闸要守的是天天看得见的界面文字。
  #  (b) **只在引号内的字符串字面量里判**。中文注释里合法地写着
  #      「一场会传了 4 段录制时」（activity-minutes.tsx），而且那是**块注释的续行**、
  #      行首没有 `//`，上面那个注释过滤器漏得掉它 → 又是假红。
  #      代价：JSX 裸文字（`<span>这场会</span>`）扫不到。
  #
  # 两处都是**故意漏**。这个脚本的原则是「宁可漏，不可假红」——
  # ★一道天天报假红的闸，两周之内就会被所有人习惯性忽略，那才是真的没有闸。★
  case " $* " in *" web/src"*|*" web/src/ "*)
    zh=$(grep -rnE '[一这那每某上下本该此]场会|场会议' web/src/ 2>/dev/null \
      | grep -v 'no-meeting:allow' \
      `# ⚠★行首是注释的先滤掉★(2026-08-12 本闸自己报了一次假红):` \
      `# tz.ts 有一行 \`/// ★只给布局计算用★(\\\`schedule-layout\\\` 要算「这场会落在哪一天…」)\`,` \
      `# 它是注释,但同一行有**反引号**(markdown 式的代码标记),` \
      `# 于是下面「引号内才算」的启发式把它当成了模板字符串 → 假红。` \
      `# ★中文注释里写反引号是本仓的常态,所以这一道必须有。★` \
      | grep -vE '^[^:]+:[0-9]+: *(//|\*|#)' \
      | grep -P "['\"\`][^'\"\`]*([一这那每某上下本该此]场会|场会议)") || true
    if [ -n "$zh" ]; then
      echo "★用户可见文案里还有「会」★（「会议→活动」当初只换了 ASCII 的 meeting）"
      echo "$zh"; rc=1
    fi ;;
  esac
  [ $rc -eq 0 ] || exit 1
