...
const 安全可点 = /^(今\s*天|‹|›|周|月|列表|展开|收起|返回|新\s*建|\+?\s*发起活动|回收站( \d+)?|文档|成员|活动|设置|信息与人员|纪要正文|个人面板|我的分享|我的活动类型|开发者|近 7 天|全部|上传文件|接着写\s*›|去整理\s*›|还有 .* 展开 ▾|.*\(无文字\))$|展开|收起/
/// full 模式（沙箱账号专用）下仍然不点的：会把页面带走或改**账号**状态的
...
/// ★默认只读★:非测试账号一律只读,想全点必须显式 AUDIT_MODE=full 且用沙箱账号。
/// 默认值站在「不会弄坏真人数据」那一边 —— 前两次事故都是因为默认站错了边。
const MODE = process.env.AUDIT_MODE ?? (/^e2e/.test(WHO) ? 'full' : 'readonly')
