...
async function 巡一屏(页面, 复位) {
  ...
  console.log(`\n── ${页面}：${全部.length} 个可点元素 ──`)
  // ★先给这一屏的**初始态**截一张★(2026-09-03 全量巡检时发现的盲区):
  //   在此之前只有第一屏有「初始」截图,进了别的视图之后、点任何东西之前的样子
  //   **一张都没有** —— 而「页面加载时发生的失败」(报告里那个「没人认领的失败」段)
  //   恰恰只发生在这一刻。那一轮报了一条 `403 GET /api/_dev/apis`,
  //   ★而它没有任何截图可看★:到底是空白页还是有提示,只能事后自己再跑一遍去截。
  //   一张图的成本,换「异常有据可查」。
  await shot(`${页面}-初始`)
