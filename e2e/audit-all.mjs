...
async function 巡一屏(页面, 复位) {
  ...
  // ★复位只经这一个入口★(2026-09-07 第二次修):#198 那笔我兜住了**顶部**这次复位,
  //   却漏了下面重试循环里的第二次 —— 于是第九轮又被一次 `nav('项目')` 超时整轮带走。
  //   ★同一个 PR 的标题写着「一次点击超时把整轮巡检带走」,而我只修了两个调用点中的一个。★
  //   抽成函数是为了让「还有没有第三个裸调用」这件事**一眼看得出来**:
  //   全文件搜 `复位(` 应当只剩这个包装器内部一处。
  const 安全复位 = async () => {
    try { await 复位(); return null } catch (e) {
      return String(e.message ?? e).split('\n')[0].slice(0, 120)
    }
  }
  const 首次失败 = await 安全复位()
  if (首次失败 !== null) {
  ...
  for (const i of 顺序) {
    ...
    for (let 轮 = 0; 轮 < 2 && !el; 轮++) {
      // ★就是这一次没兜住,害得第九轮整轮没了★:复位失败只该让**这一个元素**判「未测」,
      //   不该把后面几百处点击和报告一起带走。
      复位失败 = await 安全复位()
      if (复位失败 !== null) break
    ...
    if (!el) {
      // ★两种「未测」要分开说★:「复位失败」和「复位成功但找不到这个元素」
      //   是两件事,混成一句话的话,看报告的人没法判断该去查环境还是查界面。
      const 因 = 复位失败 !== null ? `未测(复位失败:${复位失败})` : '未测(复位后找不到这个元素)'
      记录.push({ 页面, 序: i, 元素: 名, 结果: 因, 截图: '' })
      console.log(`  ? [${页面}] #${i} ${名} — ${因}`); continue
    }
...
try {
  ...
  if (r.ok) {
    活动清单 = (await r.json())
      ...
      // ★带上 id★:下面用 `?activity=<id>` 深链直接进详情页,不再去列表里按标题找 ——
      //   见 for 循环里那段注释,按标题找是这道审计上一次「假覆盖」的成因。
      .map((a) => ({ id: a.id, 名: a.title })).filter((x) => x.id && x.名).slice(0, 6)
  } else {
...
for (const { id: 活动id, 名 } of 活动清单) {
  await 巡一屏(`活动·${名}`, async () => {
    // ★用深链 `?activity=<id>` 直接进详情页★(2026-09-04 第二次修这一段)。
    //
    // 上一版是「回首页 → 点『活动』→ 在列表里 `getByText(名)` → 点它」,而且
    // ★找不到就什么都不做、留在原地★。活动列表的默认页签是「我参与的」,那一格
    // 恰好是 `共 0 场` —— 标题压根不在 DOM 里 → count()==0 → 不点 →
    // ★六场「活动详情」拍的全是同一张空列表页★,报告却写着「活动共巡 6 场、未测 0 处」。
    //
    // ★这比上一轮的「共巡 0 场」更坏:那次报告只是漏了,这次报告**撒了谎**★ ——
    // 它声称了自己并没有的覆盖,而截图文件名叫「活动·八月第一次组会」,
    // 不逐张看是看不出来的。这也正是「★脚本跑完必须人逐张看截图★」这条纪律的由来。
    //
    // ⇒ 不再依赖「某个页签下恰好列出了这场活动」这种**当时才知道**的前提。
    //   `?activity=<id>` 是站内信深链用的既有入口(app.tsx `deepLinkActivityId`),
    //   与列表页签无关,拿 API 回的 id 直接进。
    await p.goto(`${BASE}/?activity=${活动id}`, { waitUntil: 'domcontentloaded' })
    ...
    // ★到没到必须**断言**,到不了要报异常★:这一段两次翻车都是「没到也不吭声」——
    //   第一次是清单为空、循环一次没进;第二次是进了循环却停在列表页。
    //   ⚠★判据本身要先验过★:我第一版写的是「信息与人员」—— 那是**纪要页**的面板名,
    //     活动详情页根本没有这四个字,这条断言会对着**进对了**的页面报异常。
    //     ★一个会误报的门禁会被人忽略,等于没有★,所以先拿探针对着真页面数了一遍:
    //     详情页顶上那个「返回活动」按钮才是它独有的(列表页没有)。
    if (!(await p.getByText('返回活动', { exact: false }).first().count())) {
      记录.push({ 页面: `活动·${名}`, 序: '-', 元素: '(进详情页)',
        结果: `★没能进到活动 ${活动id} 的详情页 —— 这一场没巡到,别把它算进覆盖★`,
        截图: await shot(`活动${名}-进不去`) })
    }
  })
}
