...
//   ② ★通知点进去要能到那场会★ —— `?meeting=<id>` 深链落地在会议上，
...
async function newMeeting(req: APIRequestContext, pid: number, title: string) {
  const now = Date.now()
...
test.describe('站内信:不拖垮业务', () => {
  test('★建会 / 改时间 / 取消都照常 200★', async ({ request }) => {
    ...
    const mid = await newMeeting(request, pid, `E2E 通知会议 ${Date.now()}`)

    // 改时间会触发一轮通知；通知失败只该 warn 一行日志，绝不能让改期失败
    const now = Date.now()
    const upd = await request.put(`/api/meetings/${mid}`, {
      data: {
    ...
    const del = await request.delete(`/api/meetings/${mid}`)
    expect(del.status(), await del.text()).toBe(200)
  })

  test('改时间把所有人的答复清回 pending', async ({ request }) => {
    const pid = await newProject(request, `E2E-改期清答复-${Date.now()}`)
    const mid = await newMeeting(request, pid, `E2E 改期 ${Date.now()}`)
    // ⚠ 详情是 { meeting, participants, projects, can_edit } 的嵌套结构,
    // 我的答复在 meeting.my_status 上(2026-08-07 第一版写成 d.my_status 挂了 —— 是 spec 错不是产品坏)
    let d = await (await request.get(`/api/meetings/${mid}`)).json()
    expect(d.meeting.my_status, '发起人建会时自动 accepted').toBe('accepted')

    const now = Date.now()
    await request.put(`/api/meetings/${mid}`, {
      data: {
        starts_at: new Date(now + 172800_000).toISOString(),
        ends_at: new Date(now + 176400_000).toISOString(),
      },
    })
    d = await (await request.get(`/api/meetings/${mid}`)).json()
    // ★改的人自己不用重新答复★（他知道自己改成了什么）——被清的是**别人**的答复。
    // 这条钉的是「别把改期人自己也清掉」，否则他每改一次时间都要给自己点一次接受。
    expect(d.meeting.my_status).toBe('accepted')
  })
})

test.describe('站内信:点得进去', () => {
  test('★?meeting= 深链直接落在那场会上★', async ({ page, request }) => {
    const pid = await newProject(request, `E2E-深链-${Date.now()}`)
    const title = `E2E 深链会议 ${Date.now()}`
    const mid = await newMeeting(request, pid, title)
