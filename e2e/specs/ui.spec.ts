...
//
// 起因很具体:2026-08-07 用户在真实界面上点开「发起会议」,发现**记录员下拉框「暂无数据」**,
// 而记录员是必填 —— 等于根本建不了会议。而后端 `/api/users` 完全正常:
...
test.describe('日程页', () => {
  ...
  test('★重叠的会议必须都看得见★', async ({ page }) => {
    ...
    for (const b of narrow) {
      ...
      // ★同一位置同宽度的多个盒子,left 必须互不相同★——相同就是叠在一起,等于有会议看不见。
      // 这是 v0.4.2 修的那个 bug(三个以上重叠时后来者全宽覆盖)。
      expect(lefts.size, '有事件盒子叠在同一位置 = 会议在界面上消失了').toBe(sameSpot.length)
    }
  })
})
